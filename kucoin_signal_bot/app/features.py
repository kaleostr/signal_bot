...
def add_indicators(df: pd.DataFrame, opts: Dict[str, Any], tf_tag: str = "") -> pd.DataFrame:
    if df.empty:
        return df

    def g(key, default):
        if tf_tag:
            v = opts.get(f"{key}_{tf_tag}")
            if v is not None:
                return v
        return opts.get(key, default)

    efast = int(g("ema_fast",20)); emid = int(g("ema_mid",50)); eslow = int(g("ema_slow",200))
    for p in (efast, emid, eslow):
        df[f"ema{p}"] = EMAIndicator(close=df["close"], window=p).ema_indicator()

    macd = MACD(close=df["close"],
                window_slow=int(g("macd_slow",26)),
                window_fast=int(g("macd_fast",12)),
    ...
    # always provide ema20/50/200 convenience
